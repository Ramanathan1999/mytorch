...
class RNNPhonemeClassifier(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        ...
        self.rnn = [ RNNCell(input_size, hidden_size) if i == 0  else RNNCell(hidden_size, hidden_size) for i in range(num_layers)]
    ...
    def forward(self, x, h_0=None):
        ...
        for t in range(seq_len):
            ...
            for l in range(self.num_layers):
                ...
                #cur_hid= self.hiddens[-1][l]
                #hidden = self.rnn[l].forward(a,cur_hid)
                x = self.rnn[l].forward(a,self.hiddens[-1][l])
                a = x
                hidden[l]= x
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for t in range(seq_len-1,-1,-1):
            ...
            for l in range(self.num_layers-1,-1,-1):
               ...
               dx,dh[l] = self.rnn[l].backward(dh[l],self.hiddens[t+1][l], h_prev_l,self.hiddens[t][l])
               #dh[l]= dhn
               if l!=0:
                  dh[l-1]+=dx
                ...
                #h_prev_l = self.hiddens[l-1][t+1] if l != 0 else self.x[:, t, :]
                #h, h_prev_t = self.hiddens[t + 1][l], self.hiddens[t][l]
                #dx, dh[l] = self.rnn[l].backward(dh[l], h, h_prev_l, h_prev_t)
                #if l != 0:
                #    dh[l-1] = dx if t == seq_len - 1 else dh[l-1] + dx
