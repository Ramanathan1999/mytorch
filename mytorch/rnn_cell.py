...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        A= np.matmul(x, self.W_ih.T) + self.b_ih
        
        b= np.matmul(h_prev_t, self.W_hh.T) + self.b_hh

        h_t = self.activation(A+b) # TODO
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        dz = delta* self.activation.backward(state= h_t) # TODO

        # 1) Compute the averaged gradients of the weights and biases
        self.dW_ih += np.matmul(dz.T,h_prev_l)/batch_size # TODO
        self.dW_hh += np.matmul(dz.T,h_prev_t)/batch_size # TODO
        self.db_ih += np.mean(dz, axis=0) # TODO
        self.db_hh += np.mean(dz, axis=0) # TODO

        # # 2) Compute dx, dh_prev_t
        dx        = np.matmul(dz, self.W_ih) # TODO
        dh_prev_t = np.matmul(dz, self.W_hh) # TODO
