...
class Dropout2d(object):
    ...
    def forward(self, x, eval=False):
        ...
        if eval == False:
            mask = np.random.binomial(np.ones((x.shape[0],x.shape[1]),dtype=np.int8),1-self.p)
            self.mask = mask.reshape(x.shape[0],x.shape[1],1,1)
            x= (np.multiply(x,self.mask)/(1-self.p))
            return x
