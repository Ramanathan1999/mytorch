...
class CTC(object):
	...
	def extend_target_with_blank(self, target):
		...
		skip_connect = [self.BLANK for i in range(N)] 
...
class CTCLoss(object):
	...
	def forward(self, logits, target, input_lengths, target_lengths):
		...
		for batch_itr in range(B):
			...
			for e in range(len(extended_symbols)):
				total_loss[batch_itr] -= np.sum(gamma[:, e] * np.log(logit[:, extended_symbols[e]]))
	...
	def backward(self):
		...
		for batch_itr in range(B):
			...
			gamma = self.ctc.get_posterior_probs(alpha, beta)
			for e in range(len(extended_symbols)):
				dY[:self.input_lengths[batch_itr], batch_itr, extended_symbols[e]] -= gamma[:, e] / logit[:, extended_symbols[e]]
